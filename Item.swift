...
class Item: NSObject {
    ...
    init(key: Int, dictionary: Dictionary<String, AnyObject>) {
        ...
        if let newSharedWith = dictionary["sharedWith"] as? Array {
            self._sharedWith = newSharedWith
        }
        
        if let newIngredients = dictionary["ingredients"] as? Array {
        ...
        self._itemRef = FirebaseProxy.firebaseProxy.itemRef.child(self._key)
